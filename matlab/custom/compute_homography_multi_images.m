...
img3Double                 = im2double(img3);
img3Gray               = rgb2gray(img3Double);

h_mat = computeHomography(img1Gray,img2Gray,img1Double,img2Double);

t = maketform('projective',h_mat');
%imgcombined12 = stich_images(img1Double,img2Double,t);
imgcombined12 = stitch_images(img1Double,img2Double,t);
imgcombined12Gray               = rgb2gray(imgcombined12);


h_mat2 = computeHomography(imgcombined12Gray,img3Gray,imgcombined12,img3Double)
t2 = maketform('projective',h_mat2');
%imgcombined123 = stich_images(imgcombined12,img3Double,t2);
imgcombined123 = stitch_images(imgcombined12,img3Double,t2);
figure,imshow(imgcombined123);

function h_mat = computeHomography(img1Gray,img2Gray,img1Double,img2Double)
numPoints = 200;
...
if(z2>1 && z2>1)
 ...
 function overlaped_img = overlapImages(xdataf,ydataf,img1,img1Transformed,img2)
    [y1 x1 z1] = size(img1);
    [y2 x2 z2] = size(img2);
    x_final_img = ceil(xdataf(2)-xdataf(1))+1;
    y_final_img = ceil(ydataf(2)-ydataf(1))+1;
